...
Abra a página de Releases e baixe `FFXMusic-Setup-1.0.2.exe`. O instalador inclui o runtime e as ferramentas de áudio e requer permissão de administrador. O destino padrão é `C:\Program Files\FFX Apps\FFX Music`. Ao desinstalar, a pasta compartilhada `FFX Apps` é preservada.
...
Os instaladores ficam nos anexos de **Releases**, e não em commits Git. O código está em `ffx-music-source`. A versão atual usa a tag `v1.0.2`. O verificador também reconhece as revisões anteriores `v1.0.0.N`.
